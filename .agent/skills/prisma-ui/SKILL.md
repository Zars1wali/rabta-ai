...
description: >
  ...
  PRISMA primitives, or anything in a `@rabta-ai/ui-domain` package.
...
```
packages/rabta-ai-ui-domain/        ← e.g. @rabta-ai/ui-domain
├── package.json
├── tsconfig.json
├── README.md
└── src/
...
`@rabta-ai/ui-domain` declares `@prisma/ui` as a `workspace:*`
...
4. Replace the import in the consuming code: from @rabta-ai/ui-domain
   to @prisma/ui.
5. Remove the component from @rabta-ai/ui-domain.
```
...
imports from `@prisma/ui` and `@rabta-ai/ui-domain` for the stories
